...
class Parser {
  ...
  /** @param {boolean=} useGeneralizedTypeTemplate */
  parseArgumentsTemplate(useGeneralizedTypeTemplate) {
    ...
    while (!this.consume(closeParen)) {
      ...
      const name = this.at("NAME") ? this.expect("NAME").val : null;
      const cls = useGeneralizedTypeTemplate ?
...
function annotate(modules) {
  ...
  function resolveExpressionList(node, bindings, stack) {
    ...
    if (varexpr !== null) {
      ...
      switch(kind) {
      case "NAME": {
        const types = getVariableType("..." + name, stack);
